    ...
    while True:
        ...
        for room_hash in to_delete:
            ...
            if room_hash in manager.rooms:
                ...
                # 回收连接池
                del manager.rooms[room_hash]
            
            # 回收内存
            if room_hash in DYNAMIC_ROOMS:
                del DYNAMIC_ROOMS[room_hash]
...
async def api_generate_room(data: dict = Body({}), x_session_token: Optional[str] = Header(None)):
    if not verify_session(x_session_token):
        raise HTTPException(401, detail="Invalid Session")
